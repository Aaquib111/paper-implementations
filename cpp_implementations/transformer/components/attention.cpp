...
struct AttentionImpl : torch::nn::Module {
    ...
    torch::Tensor forward(torch::Tensor x) {
        ...
        torch::Tensor q = torch::matmul(x.unsqueeze(1), weight_q).permute({0, 2, 1, 3}) + bias_q;

        // Key vector, shape (batch, seq_pos, n_heads, d_head)
        torch::Tensor k = torch::matmul(x.unsqueeze(1), weight_k).permute({0, 2, 1, 3}) + bias_k; 

        // Apply RoPE to q and k
        q = apply_rotary(q);
        k = apply_rotary(k);

        // Calculate attention scores = qk^T, of shape (batch, n_heads, seq_pos, seq_pos)
        torch::Tensor attn_scores = torch::matmul(q, k.permute({0, 1, 3, 2})) / std::sqrt(d_head);
        ...
        attn_pattern = torch::where(attn_pattern.isnan(), torch::zeros_like(attn_pattern), attn_pattern);

        // Value vector, shape (batch, seq_pos, n_heads, d_head)
        torch::Tensor v = torch::matmul(x.unsqueeze(1), weight_v).permute({0, 2, 1, 3}) + bias_v; 

        // Attention output before applying weight_o, shape (batch, query_pos, n_heads, d_head) 
        torch::Tensor z = torch::matmul(attn_pattern, v);

        // Pass through weight_o to get final attention output
        torch::Tensor attn_out = torch::matmul(
            z.reshape({z.size(0), z.size(1), -1}),
